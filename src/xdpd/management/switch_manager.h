...
public:
	 ...
	 */
	static openflow_switch* create_switch2(of_version_t version,
					uint64_t dpid,
					std::string const& dpname,
					unsigned int num_of_tables,
	...
	/**
	 * Conversion from datapathname to datapath dpid
	 * @param dpname
	 * @return
	 * @author Daniel Depaoli <daniel.depaoli (at) create-net.org>
	 */
	static uint64_t dpid_from_name(std::string dpname);

	/**
	 * Conversion from datapath dpid to datapath name
	 * @param dpid
	 * @return
	 * @author Daniel Depaoli <daniel.depaoli (at) create-net.org>
	 */
	static std::string dpname_from_dpid(uint64_t dpid);
