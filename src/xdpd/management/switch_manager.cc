...
openflow_switch* switch_manager::create_switch2(
		of_version_t version,
		uint64_t dpid,
		std::string const& dpname,
		unsigned int num_of_tables,
	...
	ROFL_INFO("[switch_manager] Created switch without controller %s with dpid 0x%llx\n", dpname.c_str(), (long long unsigned)dpid);

	return dp;
}

uint64_t xdpd::switch_manager::dpid_from_name(std::string dpname) {

	//openflow_switch* switch_temp;

	for(std::map<uint64_t, openflow_switch*>::iterator it = switchs.begin(); it != switchs.end(); ++it) {
		if( it->second->dpname == dpname)
			return it->first;
	}
...
std::string xdpd::switch_manager::dpname_from_dpid(uint64_t dpid) {
	...
	switch_temp = switch_manager::switchs[dpid];

	return switch_temp->dpname;
}
