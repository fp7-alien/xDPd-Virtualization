...
void lsi_scope::post_validate(libconfig::Setting& setting, bool dry_run){
	...
	if(!dry_run){
		...
		// Depending on virtualization variable creates switch with or without controller
		if ( ! virtual_agent::is_active() )
		{
			ROFL_INFO("Virtualization is not active. Default switch creator\n");
		...
		{
			ROFL_INFO("Virtualization is active. Virtualization method for switch creation\n");
