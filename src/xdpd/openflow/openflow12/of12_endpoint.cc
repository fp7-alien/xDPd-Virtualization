...
*
* In virtualization case filters the port
*
*/
void
		...
		cofmsg_features_request *msg)
{
	...
	{
		slice* slice = virtual_agent::list_switch_by_id[this->of12switch->dpid]->select_slice(ctl);
...
/**
 * In virtualization case filter the ports
 * @param ctl
 * @param msg
 */
void
		...
		if (OFPP12_ALL == port_no){
			...
			for (unsigned int n = 1; n < of12switch->max_ports; n++){
				...
				if((port != NULL) && (of12switch->logical_ports[n].attachment_state == LOGICAL_PORT_STATE_ATTACHED)
						...
						// Check if port is avaible for the slice
	...
	if (virtual_agent::is_active())
	{
		...
		*/
		...
		for (unsigned int n = 1; n < of12switch->max_ports; n++){
			...
			if((port != NULL) && (of12switch->logical_ports[n].attachment_state == LOGICAL_PORT_STATE_ATTACHED)
					&& slice->has_port(of12switch->logical_ports[n].port->name)/* && (port->of_port_num == portnum)*/){
...
/**
 * In virtualization case entry is passed to VA
		...
		cofmsg_packet_out *msg)
{
	...
	of1x_action_group_t* new_action_group = new of1x_action_group_t;// = of1x_init_action_group(NULL);

	if (virtual_agent::is_active())
	{
		// Case LLDP packet
		if ( !msg->get_packet().empty() && msg->get_packet().get_match().get_eth_type() == eth_lldp )
		{
...
/**
 * In virtualization case sends packet to controller
 * selected by the virtualization agent
 *
 * @param table_id
 * @param reason
 * @param in_port
		...
		cofctl* controller)
{
	try {
		...
		if (controller == NULL && virtual_agent::is_active())
			ROFL_INFO("\n\nController NULL: if virtual agent works this case must be impossible\n\n\n");
		...
		cofmsg_flow_mod *msg) //throw (eOfSmPipelineBadTableId, eOfSmPipelineTableFull)
{
	...
	if (virtual_agent::is_active())
	{
		...
		{
			send_error_message(ctl, msg->get_xid(), OFPET_BAD_MATCH, OFPBAC_MATCH_INCONSISTENT, msg->soframe(), msg->framelen());
			return;
		}
		catch(...)
		{
		...
		bool strict)
{
	...
	if (virtual_agent::is_active())
	{
		...
		{
			send_error_message(ctl, pack->get_xid(), OFPET_BAD_MATCH, OFPBAC_MATCH_INCONSISTENT, pack->soframe(), pack->framelen());
			return;
		}
		catch(...)
		{
...
/**
 * Virtualization change group id
 * @param ctl
 * @param msg
 */
void
		...
		cofmsg_group_mod *msg)
{
	...
	switch(msg->get_command()){
		case OFPGC_ADD:
			...
			ret_val = fwd_module_of1x_group_mod_add(sw->dpid, (of1x_group_type_t)msg->get_group_type(),
		...
		case OFPGC_MODIFY:
			of12_translation_utils::of12_map_bucket_list(ctl, sw, msg->get_buckets(), bucket_list);
			ret_val = fwd_module_of1x_group_mod_modify(sw->dpid, (of1x_group_type_t)msg->get_group_type(),
					(virtual_agent::is_active())?*new_group_id:msg->get_group_id(),
					bucket_list);
			break;
		
		case OFPGC_DELETE:
			ret_val = fwd_module_of1x_group_mod_delete(sw->dpid,
					(virtual_agent::is_active())?*new_group_id:msg->get_group_id());
			break;
...
of12_endpoint::handle_ctrl_close(cofctl *ctrl)
{
	...
	{
		slice* slice = virtual_agent::list_switch_by_id[sw->dpid]->select_slice(ctrl);
		ROFL_INFO("[sw: %s] Slice %s (%s:%u) has DISCONNECTED. \n", sw->dpname.c_str() ,slice->name.c_str(),ctrl->get_peer_addr().c_str());
	}
	else
...
xdpd::of12_endpoint::of12_endpoint(openflow_switch* sw) throw (eOfSmErrorOnCreation) :
		of_endpoint(1 << OFP12_VERSION) {
