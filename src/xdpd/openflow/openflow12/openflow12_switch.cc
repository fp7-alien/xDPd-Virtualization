...
*/
void openflow12_switch::rpc_connect_to_ctl(caddress const& controller_addr, cofctl* ctl){
	return endpoint->rpc_connect_to_ctl(ofswitch->of_ver, 0, controller_addr);
}

cofctl* openflow12_switch::rpc_connect_to_ctl_return(caddress const& controller_addr, cofctl* ctl)
{
...
xdpd::openflow12_switch::openflow12_switch(uint64_t dpid,
		const std::string& dpname, unsigned int num_of_tables, int* ma_list)
				throw (eOfSmVersionNotSupported): openflow_switch(dpid, dpname, version)
{
	...
	//Initialize the endpoint without launch controller
