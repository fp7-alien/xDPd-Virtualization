		...
		cofmsg_features_request *msg)
{
	...
	{
		slice* slice = virtual_agent::list_switch_by_id[this->of10switch->dpid]->select_slice(ctl);
		...
		*/
		...
		for (unsigned int n = 1; n < of10switch->max_ports; n++){
			...
			if( of10switch->logical_ports[n].attachment_state == LOGICAL_PORT_STATE_ATTACHED /* && (port->of_port_num == portnum)*/
					&& slice->has_port(of10switch->logical_ports[n].port->name)){
		...
		cofmsg_packet_out *msg)
{
	...
	try{
		of10_translation_utils::of1x_map_flow_entry_actions(ctl, sw, msg->get_actions(),
				action_group, NULL);//TODO: is this OK always NULL?
	}catch(...){
...
/**
 * Virtualization agent analyse and modify entry
...
of10_endpoint::of10_endpoint(openflow_switch* sw) throw (eOfSmErrorOnCreation) :
	of_endpoint(1 << OFP10_VERSION) {
